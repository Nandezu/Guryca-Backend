...
def delete_try_on_result(request, result_id):
    try:
        ...
        logger.info(f"Deleting result {result_id} for user {request.user.id}")
        ...
        result.delete()
        logger.info("Result deleted from database")
        return Response(status=status.HTTP_204_NO_CONTENT)
    except TryOnResult.DoesNotExist:
        logger.warning(f"Try-on result {result_id} not found for user {request.user.id}")
