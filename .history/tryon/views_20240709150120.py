...
def delete_try_on_result(request, result_id):
    try:
        ...
        if result.result_image:
            try:
                # Pokud result_image je řetězec (URL), extrahujte název souboru
                if isinstance(result.result_image, str):
                    file_name = result.result_image.split('/')[-1]
                else:
                    file_name = result.result_image.name
                
                default_storage.delete(file_name)
            except Exception as e:
                logger.error(f"Chyba při mazání souboru: {str(e)}")
                # Pokračujeme v mazání záznamu i když se nepodaří smazat soubor
        ...
        logger.error(f"Neočekávaná chyba při mazání try-on výsledku: {str(e)}")
