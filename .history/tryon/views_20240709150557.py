...
def delete_try_on_result(request, result_id):
    try:
        ...
        # Logování URL obrázku před smazáním
        logger.info(f"Deleting TryOnResult with id: {result_id}, image URL: {result.result_image}")
        
        # Smazání záznamu z databáze
        result.delete()
        logger.info(f"Successfully deleted TryOnResult with id: {result_id}")
        
        return Response(status=status.HTTP_204_NO_CONTENT)
    except ObjectDoesNotExist:
        logger.warning(f"TryOnResult with id {result_id} not found for user {request.user.id}")
        return Response({"error": "Try-on result not found"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        logger.error(f"Unexpected error when deleting try-on result: {str(e)}")
