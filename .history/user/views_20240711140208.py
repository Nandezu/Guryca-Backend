...
def change_region(request):
    ...
    logger.info(f"Change region request for user {user.id}: new_region={new_region}")

    if not new_region:
        logger.warning(f"Change region failed for user {user.id}: New region is required.")
        return Response({'error': 'New region is required.'}, status=status.HTTP_400_BAD_REQUEST)

    user.region = new_region
    user.save()
    
    logger.info(f"Region changed successfully for user {user.id} to {new_region}")
