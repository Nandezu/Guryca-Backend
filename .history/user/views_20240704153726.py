...
class CustomUserViewSet(viewsets.ModelViewSet):
    ...
    def upload_profile_image(self, request, pk=None):
        ...
        # Generujeme konzistentní název souboru
        file_name = f"photo_{uuid.uuid4().hex[:8]}{os.path.splitext(image.name)[1]}"
        
        # Nastavení cesty pro Django
        django_file_path = f"profile_images/{file_name}"
        
        # Nastavení cesty pro S3
        s3_file_path = f"profile_images/{user.id}/{file_name}"
        
        # Uložení do Django SQLite databáze
        user.profile_image = django_file_path
