...
class CustomUserViewSet(viewsets.ModelViewSet):
    ...
    def upload_profile_image(self, request, pk=None):
        ...
        # Kontrola limitu profilových obrázků
        subscription_limits = {
            'free': 3,
            'basic': 7,
            'pro': 12,
        ...
        max_images = subscription_limits.get(user.subscription_type, 3)
        if len(user.profile_images) >= max_images:
            logger.warning(f"User {pk} has reached the profile image limit.")
