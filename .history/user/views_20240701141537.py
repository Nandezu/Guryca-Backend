...
def login(request):
    ...
    print(f"Login attempt: username={username}, password={'*' * len(password)}")  # Pro debugging
    ...
    if user is None:
        print("Authentication failed")  # Pro debugging
        return Response({'error': 'Invalid Credentials'}, status=status.HTTP_401_UNAUTHORIZED)

    print(f"User authenticated: {user.username}")  # Pro debugging
