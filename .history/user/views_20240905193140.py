        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price_data': {
                    'currency': 'usd',
                    'product_data': {
                        'name': '50 Virtual Try-On Credits',
                    },
            ...
            mode='payment',
            success_url=settings.FRONTEND_URL + f'/success?session_id={{CHECKOUT_SESSION_ID}}&user_id={user.id}',
...
def confirm_payment(request):
    ...
    try:
        ...
        if session.payment_status == 'paid':
            ...
            user.virtual_try_ons_remaining += 50  # Add 50 credits
            user.save()
            return JsonResponse({'status': 'success', 'credits_added': 50})
        else:
