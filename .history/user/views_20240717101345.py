...
def use_feature(request):
    ...
    if feature_type == 'virtual_try_on':
        if user.virtual_try_ons_remaining > 0:
            user.virtual_try_ons_remaining -= 1
            user.save()
            return Response({'message': 'Virtual try-on used successfully'})
        else:
            return Response({'error': 'No virtual try-ons remaining'}, status=status.HTTP_403_FORBIDDEN)
    elif feature_type == 'profile_image':
        if user.profile_images_remaining > 0:
            user.profile_images_remaining -= 1
            user.save()
            return Response({'message': 'Profile image slot used successfully'})
        else:
            return Response({'error': 'No profile image slots remaining'}, status=status.HTTP_403_FORBIDDEN)
    elif feature_type == 'try_on_result':
        if user.try_on_results_remaining > 0:
            user.try_on_results_remaining -= 1
            user.save()
            return Response({'message': 'Try-on result used successfully'})
        else:
            return Response({'error': 'No try-on results remaining'}, status=status.HTTP_403_FORBIDDEN)
    else:
        return Response({'error': 'Invalid feature type'}, status=status.HTTP_400_BAD_REQUEST)
