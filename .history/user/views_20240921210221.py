...
def redirect_to_payment(request):
    payment_link_url = "https://buy.stripe.com/bIY9BW6PxcCTfvObIT"
...
def handle_successful_payment(session):
    ...
    logger.info(f"Handling successful payment for session {session['id']}")
    ...
    if client_reference_id:
        try:
            ...
            logger.error(f"User not found with id: {client_reference_id}")
    else:
    ...
    plans = [
        {
            'name': 'Free Plan',
            'price': '$0',
            'features': ['5 VIRTUAL TRY-ONS', '7 PROFILE IMAGES', '200 TRY-ON RESULTS'],
            'ai_section': False,
            'new_section': False,
            'product_id': None
        },
