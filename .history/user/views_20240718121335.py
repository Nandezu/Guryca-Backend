...
def purchase_subscription(request):
    ...
    return Response({
        'message': f'{plan_type.capitalize()} plan purchased successfully',
...
def use_feature(request):
    ...
    print(f"Is subscription active: {user.is_subscription_active}")
    print(f"Feature type requested: {feature_type}")

    success, message = user.use_feature(feature_type)
    print(f"Use feature result: success={success}, message={message}")
...
def change_subscription(request):
    ...
    return Response({
        'message': f'Subscription changed to {new_plan_type} successfully',
        'subscription_details': CustomUserSerializer(user).data
    }, status=status.HTTP_200_OK)
...
def manual_subscription_update(request):
    ...
    return Response({
        'message': f'Subscription manually updated to {new_plan_type} for user {user.username}',
        'subscription_details': CustomUserSerializer(user).data
    }, status=status.HTTP_200_OK)