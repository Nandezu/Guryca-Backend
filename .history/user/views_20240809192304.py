...
def purchase_subscription(request):
    ...
    # Verify the purchase with expo-in-app-purchases
    if not verify_purchase(product_id, receipt):
...
def change_subscription(request):
    ...
    # Verify the purchase with expo-in-app-purchases
    if not verify_purchase(new_product_id, receipt):
        return Response({'error': 'Invalid purchase'}, status=status.HTTP_400_BAD_REQUEST)
...
def verify_purchase(product_id, receipt):
    # This function should verify the purchase with expo-in-app-purchases
    # Since the actual verification is done on the client-side, this is a placeholder
    # You may want to implement additional server-side verification if needed
    return True  # For now, we'll assume all purchases are valid