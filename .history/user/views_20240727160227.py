...
def change_email(request):
    ...
    if not new_email or not password:
        return Response({'error': 'Both new email and password are required.'}, status=status.HTTP_400)
...
def reset_password_request(request):
    ...
    reset_url = f"{settings.FRONTEND_URL}/reset-password/{reset_token}?email={email}"
...
def reset_password_page(request, token):
    ...
    if token and email:
        ...
        return HttpResponse('Invalid token or email', status=400)                        
